...
def carregar_arquivo():
    ...
    try:
        with open(nome_arquivo, 'r', encoding='utf-8') as arquivo:
            ...
            for linha in linhas:
                #strip() remove os espaços em branco e caracteres de nova linha do início e do fim de uma string.
...
def executar_busca_profundidade():
    ...
    while atual != ponto_final:
        ...
        if not vizinhos_validos:
            print(f"LOOP encontrado no nó {atual}.")
            print("FALHA!")
            break
        
        vizinhos_validos.sort()

        proximo_no = vizinhos_validos[0]
        custo_caminho = vizinhos[proximo_no]
        nova_distancia = distancia + custo_caminho

        print(f"\nIteracao {iteracao}:")
        print(f"Lista: ({proximo_no}: {nova_distancia} + 0 = {nova_distancia})")

        nos_expandidos += 1
        print(f"Medida de desempenho: {nos_expandidos} nó(s) expandidos")
    ...
    if sucesso:
        ...
        print("Distância: Incompleta")
        print(f"Caminho percorrido até falhar: {' - '.join(caminho)}")
        print(f"Medida de desempenho (Nós Expandidos): {nos_expandidos}")
    print("="*30)
...
def exibir_menu():
    while True:
        ...
        print("2. Executar A*")
        print("3. Executar Busca em Profundidade")
        print("4. Sair")
        print("="*40)
